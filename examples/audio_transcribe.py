...
try:
    print(f"Wit.ai thinks you said {r.recognize_wit(audio, key=WIT_AI_KEY)}")
except sr.UnknownValueError:
...
try:
    print(
        f"Microsoft Azure Speech thinks you said {r.recognize_azure(audio, key=AZURE_SPEECH_KEY)}"
    )
...
try:
    print(
        f"Microsoft Bing Voice Recognition thinks you said {r.recognize_bing(audio, key=BING_KEY)}"
    )
...
try:
    print(
        f"IBM Speech to Text thinks you said {r.recognize_ibm(audio, username=IBM_USERNAME, password=IBM_PASSWORD)}"
    )
